...
### Opinion data
Opinion data must be provided for the *Left Side* (**LS**), *mid Left Side* (**mLS**), *mid Right Side* (**mRS**), and *Right Side* (**RS**). Names (```LSname```, ```mLSname```, etc.) should be provided as text strings, but do not need to be provided if you prefer the column labels to be left blank (e.g., ```mLS``` and ```mRS``` are often left blank). If you want to have line breaks included in the eventual plot, use "```/n```". 
